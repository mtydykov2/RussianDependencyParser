...
def train_and_test_parser(filename, model_type, include_morph=None, include_lemma=None, include_detailed_postags=False, include_coarse_postags=None, no_unk=False):
    subprocess.call('echo "\n MORPH: '+str(include_morph)+'\tLEMMA: ' +str(include_lemma) + '\tDPOS: ' + str(include_detailed_postags) + '\tCPOS: ' + str(include_coarse_postags) + '\tNO_UNK: ' + str(no_unk) + '\tMODEL_TYPE: ' + str(model_type) + '" >> batch_test', shell=True) 
    ...
    subprocess.call('./TurboParser-2.1.0/scripts/eval.pl -q -g all_test.conll.coarse.parser_input -s russian_parse.conll.predicted >> batch_test', shell=True)
...
subprocess.call('echo "****************************************************************" >> batch_test', shell=True)
subprocess.call('echo "\n\nTest on Test Corpus B:" >> batch_test', shell=True)
